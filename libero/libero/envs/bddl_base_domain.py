...
SIMPLIFY = {
    'pick up the bowl and place it back on the plate': {
        'akita_black_bowl_1 on plate_1': 'bowl 1 on plate 1',
    },
    'lift the bottle and put it down on the plate': {
        'wine_bottle_1 on plate_1': 'bottle 1 on plate 1',
    },
    ...
    'swap the 2 bowls on their plates using the empty plate': {
        'akita_black_bowl_1 on plate_1': 'bowl 1 on plate 1',
        'akita_black_bowl_2 on plate_2': 'bowl 2 on plate 2',
        'akita_black_bowl_1 on plate_3': 'bowl 1 on plate 3',
    },
    'rotate the 3 bowls on their plates from left to right using the empty plate': {
        'akita_black_bowl_1 on plate_1': 'bowl 1 on plate 1',
        'akita_black_bowl_2 on plate_2': 'bowl 2 on plate 2',
        'akita_black_bowl_3 on plate_3': 'bowl 3 on plate 3',
        'akita_black_bowl_1 on plate_4': 'bowl 1 on plate 4',
    },
    'put the cream cheese in the nearest basket and place that basket in the center': {
        'cream_cheese_1 in basket__contain_region_1': 'cream cheese 1 in basket 1',
        'basket_1 on kitchen_table_the_center_1': 'basket 1 in center',
    },
...
class BDDLBaseDomain(SingleArmEnv):
    ...
    def simplify_goalrep(self, subgoal_text, task_text=None):
        if task_text is None:
            return subgoal_text
        return SIMPLIFY[task_text][subgoal_text]
        
    def get_satisfied_subgoals(self, task_text=None):
        naming_cnt = {}
        renamer = {}
        satisfied_subgoals = []

        for cnt, subgoal in enumerate(self._satisfied_subgoals):
            subgoal_keys = subgoal[1]
    
            word1 = re.sub(r'\d+', '', subgoal_keys[1])
            if word1 not in naming_cnt:
                naming_cnt[word1] = 0
            word2 = re.sub(r'\d+', '', subgoal_keys[2])
            if word2 not in naming_cnt:
                naming_cnt[word2] = 0

            if subgoal_keys[1] not in renamer:
                naming_cnt[word1] += 1
                renamer[subgoal_keys[1]] = word1
                if word1[-1] != '_':
                    renamer[subgoal_keys[1]] += '_'
                renamer[subgoal_keys[1]] += str(naming_cnt[word1])
            if subgoal_keys[2] not in renamer:
                naming_cnt[word2] += 1
                renamer[subgoal_keys[2]] = word2
                if word2[-1] != '_':
                    renamer[subgoal_keys[2]] += '_'
                renamer[subgoal_keys[2]] += str(naming_cnt[word2])
                
            subgoal_text = renamer[subgoal_keys[1]] + ' ' + subgoal_keys[0] + ' ' + renamer[subgoal_keys[2]]
            satisfied_subgoals.append(NUM_TO_WORD[cnt+1] + ': ' + self.simplify_goalrep(subgoal_text, task_text))
        return satisfied_subgoals
    ...
    def is_subgoal_trajectory_possible(self, current_traj, testing_traj):
        """ 
        Judge if the current subgoal trajectory matches the prefix of the testing trajectory.
        """
        set_a = set(tuple(item) for clause in current_traj for item in clause[1:])
        set_b = set(tuple(item) for clause in testing_traj for item in clause[1:])
