...
def playback_trajectory_with_obs(
    ...
    for i in range(traj_len):
        if video_count % video_skip == 0:
            ...
            cv2.imshow('agentview_image', cv2.resize(im[0], (300, 300)))
            cv2.imshow('robot0_eye_in_hand_image', cv2.resize(im[1], (300, 300)))
            print('obs/robot0_eef_pos', traj_grp['obs/robot0_eef_pos'][i])
            print('obs/robot0_eef_quat', traj_grp['obs/robot0_eef_quat'][i])
            print('obs/robot0_gripper_qpos', traj_grp['obs/robot0_gripper_qpos'][i])
            print('obs/robot0_gripper_qvel', traj_grp['obs/robot0_gripper_qvel'][i])
            _key_ = cv2.waitKey(10000)
            if _key_ == ord('n'):
                continue
            elif _key_ == ord('q'):
                break
